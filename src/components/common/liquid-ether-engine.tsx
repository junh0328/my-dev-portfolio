...
`;
    ...
    class ExternalForce extends ShaderPass {
      ...
      update(props?: {
        ...
        const { cursor_size = 0, mouse_force = 0, cellScale } = props || {};
        const forceX = (Mouse.diff.x / 2) * mouse_force;
        const forceY = (Mouse.diff.y / 2) * mouse_force;
        const cs = cellScale || { x: 1, y: 1 };
        const cursorSizeX = cursor_size * cs.x;
        const cursorSizeY = cursor_size * cs.y;
        const centerX = Math.min(
          Math.max(Mouse.coords.x, -1 + cursorSizeX + cs.x * 2),
          1 - cursorSizeX - cs.x * 2
        );
        const centerY = Math.min(
          Math.max(Mouse.coords.y, -1 + cursorSizeY + cs.y * 2),
          1 - cursorSizeY - cs.y * 2
        );
